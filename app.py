...
# Fix for model loading - must be at the top before any other TF operations
...
def classify_image(file_like, model):
    """
    Classify image and return both prediction result and confidence
    Returns:
        tuple: (classification result, confidence percentage)
    """
    try:
...
def load_classification_model():
    """
    Load the classification model with version compatibility fixes
    """
    MODEL_PATH = r"Models/vehicle_detection_h5.h5"
    try:
        # Clear any existing TensorFlow session
        tf.keras.backend.clear_session()
        
        # Custom load function to handle version differences
        ...
        try:
            ...
            st.success("Model loaded with custom InputLayer!")
            return model
...
def handle_video_upload(uploaded_file):
    """Handle new video upload and reset necessary session states"""
